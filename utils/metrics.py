...
def recall_at_k_batch(hits, k):
    ...
    hits: array, element is binary (0 / 1), 2-dim
    """
    all_pos_num = hits.sum(axis=1)
    all_pos_num[all_pos_num == 0] = np.inf
    res = hits[:, :k].sum(axis=1) / all_pos_num
    return res
...
def calc_metrics_at_k(cf_scores, train_user_dict, test_user_dict, user_ids, item_ids, Ks):
    ...
    for idx, u in enumerate(user_ids):
            ...
            #print(f"Skipping user {u} as it is not found in train_user_dict.")
            #continue
